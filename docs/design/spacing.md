...
The scale is used sparsely on purpose. A handful of steps carry almost everything: the tight pairs, the block paddings, the column gap, and the page gutters.
...
- The only arbitrary spacing values allowed are the optical corrections above, and guards that stop a column collapsing under its own content.
