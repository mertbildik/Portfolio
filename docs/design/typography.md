...
| Token | For |
|---|---|
| `text-hero` | Homepage hero statement below `md` |
| `text-hero-lg` | Homepage hero statement from `md` |
| `text-display-lg` | Case-study page headlines and every section title |
...
- Use only these eleven. No `text-xs` or `text-2xl`, and no arbitrary sizes.
- The homepage hero is the one responsive type treatment: `text-hero md:text-hero-lg`.
...
The site is a single narrow column that never grows past its cap. All sizes except the homepage hero are fixed pixels. Readability is carried by whitespace and the ink ramp, not by large type. Tracking is owned by each token: display sizes tighten proportionally with `em`, and the fixed 16–22px sizes carry small optical pixel adjustments for the same reason. There is no separate tracking step in markup.

Responsiveness lives in the layout. Do not add other responsive type variants to compensate for a weak composition.
