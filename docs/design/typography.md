...
Twelve tokens. One class sets size, weight, line height and letter spacing together, so choosing a size is one decision rather than four. The values are in `src/index.css`.
...
| `text-subhead` | A lead paragraph carrying more weight than body |
...
- Use only these twelve. No `text-xs` or `text-2xl`, no arbitrary sizes, no responsive size variants in markup.
